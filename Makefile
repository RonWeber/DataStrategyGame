CXX = g++
CXXFLAGS = -Wextra -Wall --coverage  -Wno-unknown-pragmas -std=c++11  -O3 -I /usr/include/lua5.1 -I include/ -I /usr/include/SDL2 -I /usr/include/GL -D_GNU_SOURCE=1 -D_REENTRANT  -DPRETTY_SAVE_FILES -g
LFLAGS = -lSDL2 -lSDL2_image -lGL -llua5.1-c++ -lgcov
...
clean:
	rm -rf *.o *.gch *.gcda *.gcno dataGame tests lcov_output coverage/

tests: $(OBJ) $(TEST_OBJ)
	$(CXX) -o $@ $^ $(FLAGS) $(LFLAGS)

test_main.gcda: tests
	./tests

lcov_output: test_main.gcda
	lcov -o lcov_output -c -d .

coverage/: lcov_output
	genhtml -o $@ $^
